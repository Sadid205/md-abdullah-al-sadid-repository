...
const Projects = ()=>{
    ...
    return (
        <div className="h-full w-11/12 m-auto sm:ms-auto">
            ...
            <div className="flex mt-10 flex-wrap gap-4">
                {
                    projects.map((item,index)=>(
                        <div className="w-80 hover:cursor-pointer duration-300 relative -z-60 transition-all ease-in  m-auto h-72" key={index}>
                            ...
                            <div className="absolute opacity-0 hover:opacity-100 top-0 left-0 flex transition-all ease-in duration-300 flex-col w-full h-full items-center justify-center bg- hover:bg-amber-400 ">
                                ...
                                <Link className="text-xl" to={item.github_link}><FaGithub /></Link>
                                <Link className="border hover:scale-110 border-gray-500 font-semibold text-gray-600  rounded-full px-2 text-sm" to={item.web_link}>Visit</Link>
                                </div>
                            </div>
                        </div>
                    ))
